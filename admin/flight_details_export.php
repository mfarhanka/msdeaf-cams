...
if(!in_array($direction,['arrival','departure'],true)){http_response_code(400);exit('A valid flight direction is required.');}
$directionLabel=ucfirst($direction);
$stmt=$pdo->prepare("SELECT u.country_name,u.username,f.*,(SELECT GROUP_CONCAT(CONCAT(a.first_name,' ',a.last_name) ORDER BY a.last_name,a.first_name SEPARATOR ', ') FROM delegation_flight_movement_members fm JOIN athletes a ON a.id=fm.athlete_id WHERE fm.movement_id=f.id) AS delegate_names FROM users u JOIN delegation_flight_movements f ON f.country_id=u.id WHERE u.role='country_manager' AND f.direction=? ORDER BY u.country_name,f.flight_datetime,f.id");
...
$start=static function()use(&$content,&$y,&$pageNo,$left,$top,$cols,$rh,$rows,$directionLabel):void{$pageNo++;$content="BT /F2 15 Tf {$left} {$top} Td ({$directionLabel} Flight Details Report) Tj ET
...
0 0 0 rg
";$country=trim((string)($row['country_name']?:$row['username']));$rowDirection=ucfirst((string)$row['direction']);$key=$country.'|'.$rowDirection;$groups[$key]=($groups[$key]??0)+1;$busCount=(int)($row['bus_count']??(!empty($row['bus_to_penang'])?1:0));$bus=$busCount?$busCount.' bus'.($busCount===1?'':'es').' / USD '.number_format($busCount*1000):'No';$values=[$country,$rowDirection,(string)$groups[$key],(string)($row['delegate_names']?:'Not assigned'),(string)(int)$row['pax'],(string)$row['flight_number'],flightAirportLabel($row['airport']??null),$bus,date('d M Y, H:i',strtotime($row['flight_datetime']))];$x=$left+3;foreach($cols as $n=>[, $w]){$content.=flightPdfCell($values[$n],$x,$y,$w,$n===0);$x+=$w;}$content.="0.85 0.85 0.85 RG {$left} ".($y-6)." m 816 ".($y-6)." l S
";$y-=$rh;}$finish();
...
{$xref}
%%EOF";
header('Content-Type: application/pdf');header('Content-Disposition: attachment; filename="'.$direction.'-flight-details-'.date('Y-m-d').'.pdf"');header('Content-Length: '.strlen($pdf));header('X-Content-Type-Options: nosniff');echo $pdf;