...
<div class="card shadow-sm"><div class="card-header">Invoice History</div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Invoice</th><th>Delegation</th><th>Issued</th><th>Type</th><th>Total</th><th></th></tr></thead><tbody><?php foreach($history as $i):?><tr><td class="fw-semibold"><?=htmlspecialchars($i['invoice_number'])?></td><td><?=htmlspecialchars($i['country_name']?:$i['username'])?></td><td><?=htmlspecialchars(date('d M Y H:i',strtotime($i['issued_at'])))?></td><td><?=$i['revision_of_id']?'<span class="badge text-bg-info">Revision</span>':'Original'?></td><td><?=htmlspecialchars($i['currency'])?> <?=number_format((float)$i['total_amount'],2)?></td><td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="invoice_download.php?id=<?=(int)$i['id']?>">Download PDF</a></td></tr><?php endforeach;?></tbody></table></div></div>
